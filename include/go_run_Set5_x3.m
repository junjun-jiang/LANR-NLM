...
%             conf.PPs{i} = para3*Hi*inv(Lo'*Lo + para1*diag(1./(vals(1:clustersz)).^para2)   )*Lo';   
        end
    end
...
%         im_SR0 = res{5}{1,1};
% %         figure,imshow(im_SR0)
%         psf         = fspecial('gauss', 7, 1.6);       
%         [im_SR] = improve_NL_DB(double(low{1,1}), double(img{1,1}), im_SR0, conf.scale, psf, 200);        
% %         figure,imshow(im_SR)
